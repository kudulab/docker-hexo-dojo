...
@test "/usr/bin/entrypoint.sh returns 0" {
  ...
  assert_line --partial "using hexoide:"
  assert_line "ide"
}

# This must run after entrypoint.sh, because at first 20-ide-setup-identity.sh
...
@test "/etc/ide.d/scripts/20-ide-setup-identity.sh is executable" {
  run stat -c %a /etc/ide.d/scripts/20-ide-setup-identity.sh
  assert_equal "$status" 0
